...
const AUTH_COOKIE_NAME = "cardnote_at";
...
app.post("/api/users/me/avatar", attachJwtSession, requireLoggedInUser, (req, res) => {
  const mode = getMediaUploadMode(hasPublic);
  if (!mode) {
    return res.status(503).json({ error: "未开放上传：请配置 COS 或构建生成 backend/public 目录" });
  }
...
async function main() {
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    console.error("❌ 缺少环境变量 DATABASE_URL，请配置 PostgreSQL 连接串后重启（本地测试见 backend/.env.example 与 docker-compose.yml）");
    process.exit(1);
  }
  ...
  const server = app.listen(PORT, () => {
    console.log(`cardnote-api listening on :${PORT}`);
