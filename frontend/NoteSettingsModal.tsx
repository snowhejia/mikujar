...
import { PresetTypeIcon, hasPresetTypeIcon } from "./appkit/PresetTypeIcon";
import {
  walkCollections,
...
function PresetTypeCard({
  ...
  const hasSvg = hasPresetTypeIcon(item.id);
  return (
    <div className="note-settings-modal__preset-card" role="presentation" title={label}>
      ...
      >
        {hasSvg ? (
          <PresetTypeIcon id={item.id} tint={item.tint} size={26} />
        ) : (
  ...
  const panelContent =
    settingsPanel === "general" ? (
      <div className="note-settings-modal__panel-scroll">
        ...
        <p className="note-settings-modal__label">
          {c.noteSettingsBgGradientLabel}
        </p>
        <div
          className="note-settings-modal__choice-row"
          role="group"
          aria-label={c.noteSettingsBgGradientAria}
        >
          <button
            type="button"
            className={
              "note-settings-modal__choice" +
              (notePrefs.bgGradient !== false
                ? " note-settings-modal__choice--active"
                : "")
            }
            aria-pressed={notePrefs.bgGradient !== false}
            onClick={() =>
              void persistNotePrefs({
                ...notePrefs,
                bgGradient: true,
