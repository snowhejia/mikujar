...
export type SidebarRailProps = {
  ...
  /** 顶部身份槽（头像 + 昵称），由 App 层传入 RailWorkspaceIdentity */
  userSlot?: ReactNode;
  /** 展开态下在 label 右侧显示的计数；未提供或 null 时不显示 */
  counts?: Partial<Record<RailKey, number | string>>;
};
...
export function SidebarRail(props: SidebarRailProps): ReactNode {
  const { activeKey, onPick, availability, expanded, onToggleExpanded, userSlot, counts } = props;
  ...
  const renderItem = (it: RailItemDef) => {
    ...
    return (
      <button
        ...
        aria-label={
          expanded && hasCount ? `${label} (${count})` : label
        }
