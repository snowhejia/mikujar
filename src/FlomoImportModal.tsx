...
export type FlomoImportProgress = { current: number; total: number };

type FlomoImportModalProps = {
  open: boolean;
  onClose: () => void;
  ...
  onRunImport: (
    ...
    onProgress?: (p: FlomoImportProgress) => void
  ) => Promise<number>;
};

export function FlomoImportModal({
  open,
  onClose,
  ...
  const [importProgress, setImportProgress] =
    useState<FlomoImportProgress | null>(null);
  ...
  const onDirChange = useCallback(
    async (e: ChangeEvent<HTMLInputElement>) => {
      ...
      setStatus(null);
      try {
        const parsed = await parseFlomoExportDirectory(files);
        setPreview(parsed);
        if (parsed.length === 0) {
          setStatus(c.importFlomoErrNone);
        }
      } catch {
    ...
    [c.importFlomoErrNone, c.importAppleNotesParseErr]
  );
  ...
  const onZipChange = useCallback(
    async (e: ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const parsed = await parseFlomoExportDirectory(virtualFiles);
        setPreview(parsed);
        if (parsed.length === 0) {
          setStatus(c.importFlomoErrNone);
        }
  ...
  const panel = (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="flomo-import-title"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 id="flomo-import-title" className="auth-modal__title">
          {c.importFlomoTitle}
        </h2>
